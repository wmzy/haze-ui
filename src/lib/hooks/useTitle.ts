// 视图级 document.title：进入设页标题，离开恢复「进入前」的值（即应用
// 入口 html 的静态 <title>——SPA 内没有第二处会写回它，恢复即回退入口
// 默认）。来自 painless 模板的实战沉淀。
//
// 方案选型——hook + effect，而非 React 19 的原生 <title> JSX：
// 1. 「离开恢复」是硬需求，而 React 19 的 head 管理只承诺卸载时移除
//    它自己浮升渲染的那个 <title> 元素——入口 html 的静态 <title>
//    在 React 树外，React 从未追踪，移除后不会自动写回。要恢复终究
//    得在 cleanup 里命令式赋值，<title> JSX 方案省不掉这段 effect，
//    反而多依赖一层浮升/去重语义（多视图共存时「谁后渲染谁赢」）。
// 2. <title> JSX 的核心收益在流式 SSR 的提前注入，SPA 场景收益不
//    存在；代价（约束在 React 19+）却真实。
